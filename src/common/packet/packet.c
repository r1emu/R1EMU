...
void
cryptPacketUnwrapHeader(uint8_t **packet, size_t *packetSize, CryptPacketHeader *header) {
...
void
cryptPacketGetHeader (uint8_t *packet, CryptPacketHeader *header) {
    memcpy(header, packet, sizeof(CryptPacketHeader));
}

void
packetNormalHeaderInit(PacketNormalHeader *normalHeader, uint32_t subtype, uint32_t packetSize) {
    variableSizePacketHeaderInit(&normalHeader->variableSizeHeader, BC_NORMAL, packetSize);
    normalHeader->subtype = subtype;
}

void
variableSizePacketHeaderInit(VariableSizePacketHeader *variableSizeHeader, uint16_t packetType, uint32_t packetSize) {
    serverPacketHeaderInit(&variableSizeHeader->serverHeader, packetType);
    variableSizeHeader->packetSize = packetSize;
}

void
serverPacketHeaderInit(ServerPacketHeader *serverHeader, uint16_t packetType) {
